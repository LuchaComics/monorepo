...
// OfferPaginationListResult represents the paginated list results for
...
func (impl OfferStorerImpl) newPaginationFilter(f *OfferPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl OfferStorerImpl) newPaginationFilterBasedOnString(f *OfferPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl OfferStorerImpl) newPaginationFilterBasedOnTime(f *OfferPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl OfferStorerImpl) newPaginationOptions(f *OfferPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl OfferStorerImpl) newPaginatorNextCursor(f *OfferPaginationListFilter, results []*Offer) (string, error) {
	var lastDatum *Offer
