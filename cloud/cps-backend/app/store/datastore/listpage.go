...
type StorePaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
// StorePaginationListResult represents the paginated list results for
// the associate records.
type StorePaginationListResult struct {
...
func (impl StoreStorerImpl) newPaginationFilter(f *StorePaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl StoreStorerImpl) newPaginationFilterBasedOnString(f *StorePaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl StoreStorerImpl) newPaginationFilterBasedOnTime(f *StorePaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl StoreStorerImpl) newPaginationOptions(f *StorePaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl StoreStorerImpl) newPaginatorNextCursor(f *StorePaginationListFilter, results []*Store) (string, error) {
	var lastDatum *Store
