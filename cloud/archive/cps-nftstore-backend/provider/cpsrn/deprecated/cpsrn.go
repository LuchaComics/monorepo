...
func (p cpsrnProvider) GenerateNumber(userRoleID int8, specialCollection int8, serviceType int8, count int64) (string, error) {
	// CASE 1: CPS_NFTSTORE Administration.
	if userRoleID == UserRoleRoot {
		...
		return "", fmt.Errorf("Out of numbers for CPS_NFTSTORE org with current count at %v", count)
	}
	...
	// CASE 3: Service Type
	switch serviceType {
	case ServiceTypeCPS_NFTSTORECapsuleYouGrade: // 788346-26649-5-1001
		if count <= MaxUserSubmissionsCountPerSeciton {
