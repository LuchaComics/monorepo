...
func (impl NFTStorerImpl) ListByFilter(ctx context.Context, f *NFTPaginationListFilter) (*NFTPaginationListResult, error) {
	...
	// Execute the query
	cursor, err := impl.NFT.Find(ctx, filter, options)
	if err != nil {
		return nil, err
	}
	...
	return &NFTPaginationListResult{
		Results:     results,
...
func (impl NFTStorerImpl) ListAsSelectOptionByFilter(ctx context.Context, f *NFTPaginationListFilter) ([]*NFTAsSelectOption, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
	defer cancel()
	...
	if startAfter != "" {
		...
		cursor, err := nft.FindOne(ctx, bson.M{"_id": startAfter}).DecodeBytes()
		if err != nil {
			log.Fatal(err)
		}
		options.SetSkip(1)
	...
	// Retrieve the list of items from the nft
	cursor, err := nft.Find(ctx, query, options)
	if err != nil {
		log.Fatal(err)
	}
	defer cursor.Close(ctx)

	var results = []*NFTAsSelectOption{}
