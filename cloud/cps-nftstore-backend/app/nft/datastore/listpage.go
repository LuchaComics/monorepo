...
type NFTPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
// NFTPaginationListResult represents the paginated list results for
// the associate records.
type NFTPaginationListResult struct {
...
func (impl NFTStorerImpl) newPaginationFilter(f *NFTPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl NFTStorerImpl) newPaginationFilterBasedOnTime(f *NFTPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl NFTStorerImpl) newPaginationOptions(f *NFTPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl NFTStorerImpl) newPaginatorNextCursor(f *NFTPaginationListFilter, results []*NFT) (string, error) {
	var lastDatum *NFT
