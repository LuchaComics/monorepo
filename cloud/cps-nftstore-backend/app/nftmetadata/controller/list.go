...
func (c *NFTMetadataControllerImpl) ListByFilter(ctx context.Context, f *domain.NFTMetadataPaginationListFilter) (*domain.NFTMetadataPaginationListResult, error) {
	...
	m, err := c.NFTMetadataStorer.ListByFilter(ctx, f)
	if err != nil {
...
func (c *NFTMetadataControllerImpl) ListAsSelectOptionByFilter(ctx context.Context, f *domain.NFTMetadataPaginationListFilter) ([]*domain.NFTMetadataAsSelectOption, error) {
	...
	m, err := c.NFTMetadataStorer.ListAsSelectOptionByFilter(ctx, f)
	if err != nil {
		c.Logger.Error("database list by filter error", slog.Any("error", err))
		return nil, err
	}
