...
// Provider provides an interface for abstracting `CPS_PINWS Registry Number`.
...
// NewProvider function is a contructor that returns the default `CPS_PINWS Registry Number` provider.
...
const (
	...
	ServiceTypeCPS_PINWSCapsule                    = 3
	ServiceTypeCPS_PINWSCapsuleIndieMintGem        = 4
	ServiceTypeCPS_PINWSCapsuleSignatureCollection = 5
	ServiceTypeCPS_PINWSCapsuleYouGrade            = 6
)
...
// Generates the unique `CPS_PINWS Registry Number` required for tracking submissions.
func (p cpsrnProvider) Generate(specialCollection int8, serviceType int8, userRoleID int8, count int64) (string, error) {
	...
	switch serviceType {
	case ServiceTypeCPS_PINWSCapsuleYouGrade, ServiceTypeCPS_PINWSCapsuleSignatureCollection, ServiceTypeCPS_PINWSCapsuleIndieMintGem, ServiceTypeCPS_PINWSCapsule:
		// STEP 1: Ensure the correct role was applied.
		switch serviceType {
		case ServiceTypeCPS_PINWSCapsuleIndieMintGem, ServiceTypeCPS_PINWSCapsule:
