...
type TenantPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
// TenantPaginationListResult represents the paginated list results for
// the associate records.
type TenantPaginationListResult struct {
...
func (impl TenantStorerImpl) newPaginationFilter(f *TenantPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl TenantStorerImpl) newPaginationFilterBasedOnString(f *TenantPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl TenantStorerImpl) newPaginationFilterBasedOnTime(f *TenantPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl TenantStorerImpl) newPaginationOptions(f *TenantPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl TenantStorerImpl) newPaginatorNextCursor(f *TenantPaginationListFilter, results []*Tenant) (string, error) {
	var lastDatum *Tenant
	...
	// Variable used to tenant the next cursor.
