...
type CreatePinObjectService struct {
	logger            *slog.Logger
	ipfsPinAddUsecase *usecase.IPFSPinAddUseCase
}

func NewCreatePinObjectService(logger *slog.Logger, uc1 *usecase.IPFSPinAddUseCase) *CreatePinObjectService {
	return &CreatePinObjectService{logger, uc1}
}
...
func (uc *CreatePinObjectService) Execute(ctx context.Context, req *PinObjectCreateServiceRequestIDO) (*PinObjectCreateServiceResponseIDO, error) {
	//
	// STEP 1:
	// Validation.
	//
