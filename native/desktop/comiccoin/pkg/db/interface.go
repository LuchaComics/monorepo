...
// Database interface defines the methods that can be used to interact with a key-value database.
// The database stores data in a persistent manner, allowing it to be retrieved and modified across multiple sessions.

// Get retrieves a value from the database by its key.
// The prefix and key parameters are used to construct the full key.
