...
func (uc *DownloadMetadataNonFungibleTokenUseCase) Execute(tokenID uint64, metadataURI string) (*domain.NonFungibleTokenMetadata, string, error) {
	...
	if strings.Contains(metadataURI, "ipfs://") {
		...
		return uc.executeForIPFS(tokenID, metadataURI)
	} else if strings.Contains(metadataURI, "https://") {
		uc.logger.Debug("Downloading metadata via https...",
			slog.Any("token_id", tokenID),
			slog.Any("metadata_uri", metadataURI))
		return uc.executeForHTTPS(tokenID, metadataURI)
	}
...
func (uc *DownloadMetadataNonFungibleTokenUseCase) executeForHTTPS(tokenID uint64, url string) (*domain.NonFungibleTokenMetadata, string, error) {
