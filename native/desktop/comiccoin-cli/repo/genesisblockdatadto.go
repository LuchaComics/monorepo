...
func (repo *GenesisBlockDataDTORepo) GetFromCentralAuthorityByChainID(ctx context.Context, chainID uint16) (*domain.GenesisBlockDataDTO, error) {
	modifiedURL := strings.ReplaceAll(genesisBlockDataURL, "${CHAIN_ID}", fmt.Sprintf("%v", repo.config.Blockchain.ChainID))
