...
function CustomerComicSubmissionAddStep1() {
  ...
  function onComicSubmissionCreateSuccess(response) {
    ...
    setForceURL("/c/submissions/comics/add/" + response.id);
  }
  ...
  // Apply service type limitation based on the customer store's level.
  const conditionalServiceTypeOptions = ((currentUser) => {
    if (currentUser.storeLevel === 1 || currentUser.storeLevel === 2) {
      ...
      // DEVELOPERS NOTE: Level 3 customer stores are allowed to add a
...
export default CustomerComicSubmissionAddStep1;
