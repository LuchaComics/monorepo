...
### BUILD
# docker build -f prod.Dockerfile -t rodolfossp/cps-pinws-frontend:prod-latest --platform linux/amd64 --build-arg REACT_APP_API_HOST=https://cpsdata.ca --build-arg REACT_APP_API_DOMAIN=cpsdata.ca --build-arg REACT_APP_API_PROTOCOL=https --build-arg REACT_APP_WWW_DOMAIN=cpsapp.ca --build-arg REACT_APP_WWW_PROTOCOL=https .

### TAG
# docker tag rodolfossp/cps-pinws-frontend:prod-latest rodolfossp/cps-pinws-frontend:prod-latest

### UPLOAD
# docker push rodolfossp/cps-pinws-frontend:prod-latest
